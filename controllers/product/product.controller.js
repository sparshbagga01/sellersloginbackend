...
export const createProductWithVariants = async (req, res) => {
  try {
    const { productName, productCategory, productSubCategory, brand, short_description, description, isAvailable, variants } = req.body;
    const vendorId = req.user?.user_id;
    if (!vendorId) return res.status(401).json({ message: "Vendor ID missing" });

    // Parse variants JSON
    let parsedVariants = [];
    try { parsedVariants = typeof variants === "string" ? JSON.parse(variants) : variants; } 
    catch { return res.status(400).json({ message: "Invalid variants JSON" }); }

    if (!productName || !productCategory || parsedVariants.length === 0)
      return res.status(400).json({ message: "productName, productCategory, and at least one variant are required" });
...
if (productSubCategory) {
  if (Array.isArray(productSubCategory)) {
    ...
    try {
      ...
      parsedSubCategories = productSubCategory.split(",").map(s => s.trim());
    }
  }
}
    ...
    while (await Product.findOne({ slug })) { slug = `${baseSlug}-${counter}`; counter++; }

    // Global images
    const globalFiles = req.files?.globalImages || [];
    const global_image_urls = globalFiles.map(file => `/uploads/products/default/${file.filename}`);
    ...
    const embeddedVariants = parsedVariants.map((v, idx) => {
      ...
      if (actualPrice > sellingPrice) discountPercent = parseFloat((((actualPrice - sellingPrice)/actualPrice)*100).toFixed(2));
      const finalPrice = Number((sellingPrice * (1 - discountPercent / 100)).toFixed(2));

      const variantFiles = req.files?.[`images-${idx}`] || [];
      const image_urls = variantFiles.map(file => `/uploads/products/variants/${file.filename}`);
...
export const getAllProducts = async (req, res) => {
  try {
    // Since variants are embedded, no need to populate
    ...
    // If productCategory is a string (not ObjectId), no population needed
    // If you later change it to reference Category, adjust accordingly
    res.status(200).json({
